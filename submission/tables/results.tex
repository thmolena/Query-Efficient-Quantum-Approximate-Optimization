...
These two empirical distributions weight trials equally at depth two and the 131,072-shot cap; runs with more trials contribute more points. Physical maps are recovered from the recorded model locations and centers only after verifying that every $\Delta\|B_{j,:}\|_2$ is less than half its parameter period. This bound makes the periodic displacement unique; reconstructed normalized designs also reproduce the saved weighted condition numbers. The physical map is $L=(\Delta B)^{-\mathsf T}(A^{-1})_{2:d+1,:}$. Improved conditioning or sensitivity does not by itself demonstrate useful proposals or saved shots.
