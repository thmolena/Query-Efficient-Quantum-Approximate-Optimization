...
from .provenance import CODE_ROOT, CANONICAL_DATA, digest, load_study
...
def verify(path=OUTPUT, replay=False):
    ...
    expected = {name: digest(CODE_ROOT/name) for name in SOURCES}
    if data['source_sha256'] != expected:
        raise ValueError('Transport audit source identity mismatch')
    ...
    if summarize(data, canonical) != data['summary']:
